import {
  ethers_exports
} from "./chunk-NEGTVI4S.js";
import {
  __publicField
} from "./chunk-QY3AG7D4.js";

// node_modules/@tychilabs/ugf-testnet-js/dist/index.mjs
...
Nonce: ${nonce}`
    ...
    if (!res.token)
      throw new UGFAuthError("Login failed — no token returned");
...
var Status = class {
  ...
  async poll(digest, opts = {}) {
    ...
    for (let i = 0; i < maxAttempts; i++) {
      ...
      onTick == null ? void 0 : onTick(status, i + 1);
...
var X402Payment = class {
  ...
  async sign(quote, signer, provider, opts = {}) {
    ...
    const option = registry.payment_options.find(
      (o) => {
        var _a;
        return o.type === "x402" && o.token === TYI_USD_PAYMENT_COIN && o.chains.some((c) => c.chain_id === chainId) && ((_a = o.receiver_address) == null ? void 0 : _a.toLowerCase()) === quote.payment_to.toLowerCase();
      }
    );
    if (!option) {
      throw new UGFError(
    ...
    const contract = new ethers_exports.Contract(tokenAddress, ERC3009_ABI, provider);
    ...
    const localDS = ethers_exports.TypedDataEncoder.hashDomain(domain);
    ...
    const evmDigest = ethers_exports.TypedDataEncoder.hash(
      domain,
      TRANSFER_WITH_AUTH_TYPES,
      message
    );
    const recovered = ethers_exports.recoverAddress(evmDigest, signature);
    if (recovered.toLowerCase() !== signerAddress.toLowerCase()) {
      throw new UGFSignatureError("Signature recovery mismatch");
    }
    const sig = ethers_exports.Signature.from(signature);
    return {
      digest: quote.digest,
      payment_mode: "x402",
      v: sig.v,
...
var VaultPayment = class {
  ...
  async pay(quote, signer, chainId, token) {
    ...
    const vault = new ethers_exports.Contract(entry.vault_address, vaultAbi, signer);
...
var Quote = class {
  ...
  async get(req) {
    ...
    if (!res.digest)
      throw new UGFError("Quote response missing digest", "QUOTE_ERROR");
    return res;
  }
};
...
//# sourceMappingURL=@tychilabs_ugf-testnet-js.js.map
